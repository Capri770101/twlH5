<template>
  <div class="page">
    ...
    <!-- 加载失败：可重试 / 可返回（旧版失败即整页空白且无法返回） -->
    <StateBlock
      v-else-if="!flower"
      type="error"
      emoji="🥀"
      :text="loadError || '商品信息加载失败'"
      hint="可能已下架，或网络不太给力"
      show-back
      @retry="loadDetail"
      @back="router.back()"
    ...
    <template v-if="flower">
      ...
      <div class="detail-hero">
        <FlowerImage v-if="flower.images && flower.images.length" :src="flower.images[0]" eager />
...
function onAddCart() {
  if (adding.value) return // 防连点重复加购
